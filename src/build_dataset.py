...
def create_dataset(chart_file, feature_directory, target_file):
    ...
    for line in lines:
        ...
        song_id = fields[0]
        tags = [t.replace("\n", "") for t in fields[5:]]
        base_path = os.path.join(feature_directory,
                                 fields[3]).replace(".mp3", "")
    ...
    df = df.drop(list(filter(lambda x: x.endswith("dmean"),
                             df.columns.values)),
                 axis=1)
    df = df.drop(list(filter(lambda x: x.endswith("dmean2"),
                             df.columns.values)),
                 axis=1)
    df = df.drop(list(filter(lambda x: x.endswith("dvar"), df.columns.values)),
                 axis=1)
    df = df.drop(list(filter(lambda x: x.endswith("dvar2"),
                             df.columns.values)),
                 axis=1)
